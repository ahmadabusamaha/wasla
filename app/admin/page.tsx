...
export default async function AdminPage() {
  ...
  const [users, orgs, pages, events] = await Promise.all([
    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          نظرة عامة على جميع بيانات وصلة.
        </p>
      </header>
              ...
              <TableBody>
                {(recentOrgs ?? []).map((org) => (
                  <TableRow key={org.id}>
                    ...
                    <TableCell>
                      <code dir="ltr" className="text-xs">/{org.slug}</code>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
              ...
              <TableBody>
                {(recentProfiles ?? []).map((p) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell>
                      <code dir="ltr" className="text-xs">@{p.username}</code>
                    </TableCell>
                    <TableCell>
                      {p.is_admin ? (
                        <Badge className="bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-400" variant="secondary">
                          admin
                        </Badge>
                      ) : (
